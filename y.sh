...
docker run -v `pwd`:/mnt -it --rm alfg/bento4 mp4fragment /mnt/0.mp4 /mnt/1.mp4 --fragment-duration 1000
rm -rf htdocs/dash/vod/10sec_1s;docker run -v `pwd`:/mnt -v `pwd`/mp4-dash.py:/opt/bento4/utils/mp4-dash.py -it --rm alfg/bento4 utils/mp4-dash.py --force /mnt/1.mp4 -o /mnt/htdocs/dash/vod/10sec_1s --mpd-name=manifest.mpd ;find htdocs/dash/vod/10sec_1s;cat htdocs/dash/vod/10sec_1s/manifest.mpd
./tools/run_vodanalyzer.sh  htdocs/dash/vod/10sec_1s/manifest.mpd
\mv 10sec_1s.cfg htdocs/livesim_vod_configs/
\mv 10sec_1s_video.dat htdocs/livesim_vod_configs/
\mv 10sec_1s_audio.dat htdocs/livesim_vod_configs/
sed -i -e 's/default_tsbd_secs = 300/default_tsbd_secs = 3000000000/g' htdocs/livesim_vod_configs/10sec_1s.cfg
